...
_FAKE_SYNTHESIS_WITH_VIOLATION = {
    "coachResponses": [
        {
            "replyTo": "how did I do today?",
            "text": (
                "Don't overthink it — you logged 900 cal and 82g protein. "
                "That keeps you on track for your weekly average."
            ),
            "timestamp": 1746057600000,
        }
    ],
    "highlights": ["Under calorie target by 300 cal"],
    "concerns": [],
    "mealPlan": None,
    "regimen": None,
...
class TestToneValidatorNonGating:
    def test_synthesis_returns_despite_banned_phrase(self):
        """Output containing a banned phrase must still flow through unchanged."""
        with patch("lib.invoke_day_synthesis._call_claude", return_value=_FAKE_SYNTHESIS_WITH_VIOLATION):
            result = synthesize(
                date="2026-05-01",
                profile=_PROFILE_WITH_RULES,
                totals=_TOTALS,
                goals_block=_GOALS_BLOCK,
                all_entries=[],
                coach_messages=[{"text": "how did I do today?", "timestamp": 0}],
                recent_history=[],
                plan_triggered=False,
            )
        assert result is not None
        assert len(result["coachResponses"]) == 1
        # The banned phrase must NOT have been filtered or blocked
        assert "don't overthink it" in result["coachResponses"][0]["text"].lower()

    def test_synthesis_does_not_raise_on_tone_violation(self):
        """synthesize() must not raise RuntimeError when output has tone issues."""
        with patch("lib.invoke_day_synthesis._call_claude", return_value=_FAKE_SYNTHESIS_WITH_VIOLATION):
            try:
                result = synthesize(
                    date="2026-05-01",
                ...
                pytest.fail(f"synthesize() raised RuntimeError on tone violation: {e}")

    def test_synthesis_highlights_pass_through_unchanged(self):
        """highlights and concerns must be returned as-is even if tone issues exist."""
        with patch("lib.invoke_day_synthesis._call_claude", return_value=_FAKE_SYNTHESIS_WITH_VIOLATION):
            result = synthesize(
                date="2026-05-01",
                profile=_PROFILE_WITH_RULES,
        ...
        assert result["highlights"] == ["Under calorie target by 300 cal"]
