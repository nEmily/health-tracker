...
8. **Never retroactively add coach responses to past days.** If a user message went unanswered days ago, that's part of her lived chat history. Generating fresh "answers" to old messages now feels like rewriting history — the user remembers what coach did and didn't say. Past unanswered messages stay unanswered. Fix the bug *going forward*; don't backfill past chat.
9. **Never change an existing coach response.** Once a coach response is generated and synced to the phone, the user has read it. Any later modification — even an "improvement" — is rewriting history she remembers. The chat-immutability rule applies to *content*, not just the array structure. `_merge_coach_responses` already preserves existing entries verbatim; never bypass it, never edit response text after-the-fact, never re-roll a response because a later run produces "better" text.
10. **Batch multi-message replies.** When the user sends multiple messages in a row before coach replies, coach reads them all at once and responds in ONE message — not N separate replies. The synthesis prompt already supports `respondsTo: [id1, id2, ...]`. If you ever see N user messages → N separate coach responses, that's the bug. The fix is always: ensure `unanswered_messages` is passed as a list and `coachResponses` returns ONE entry covering all of them.
