...
1. **Automated processing** — runs every 30 min via `process-day.bat`. Calls `python processing/process_day.py` which analyzes photos, computes totals, generates meal plans, and produces highlights/concerns.
...
Automated processing MUST preserve any `days[]` entry with `source` starting `coach-session` for today and future dates. Without this, interactive commits get overwritten on the next cron cycle. The preservation logic is implemented in `processing/lib/invoke_day_synthesis.py`.
...
Match the processing schema exactly: `mealPlan.days[].meals[]` with `meal` = breakfast|lunch|snack|dinner, each meal carrying `ingredients[]` with per-item cal/macros, plus meal-level totals and `day_totals`.
