...
- `{DATA_DIR}/profile/identity.md` — user's immutable identity facts, dislikes, equipment, genetic patterns (optional)
- `{DATA_DIR}/profile/current-stats.json` — **source of truth for current weight, trends, adherence** (computed artifact, regenerated by build-summary.js every processing cycle)
- DEPRECATED (ignore if present): `bio.txt`, `measurements.json` — pull all body stats from `current-stats.json` or the newest `analysis/*.json`
...
6. **Generate highlights and concerns:**
   ...
   - **Prepared-meal declaration detection.** Scan today's entries for meals logged BEFORE being eaten — signals: entry note says "prepared", "weighed", "for dinner tonight", "meal prepped"; entry timestamp is hours before typical dinner time; entry has a raw-weight note. When detected, treat the entry as a DECLARED PLAN and back-solve the remaining-day budget around it. If the user already took the shake (or other negotiable staples) AND the declared meal pushes the day over target, add a concern that: (a) acknowledges the day will land slightly over, (b) gives a forward-looking tip for next time such a meal is planned (e.g., "If you know ribeye is dinner, ping me before the afternoon shake and I'll back-solve the staples"), (c) does NOT shame the past choice. Reference: 2026-04-18 ribeye declared at 3:36 PM after shake at 3:29 PM.
